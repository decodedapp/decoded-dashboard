      ...
      <div className="max-w-[1400px] mx-auto p-6 h-auto flex flex-col">
        <div className="space-y-4">
          ...
          <div className="p-4 bg-gradient-to-b from-[#1A1A1A] to-[#222] rounded-lg border border-blue-500/10">
            <p className="text-gray-300 text-sm leading-relaxed">
              아이템에 대한 정확한 정보를 제공해주세요. 필수 정보를 입력하시면
              기본 보상을 받으실 수 있으며, 추가 정보를 입력하시면 더 많은
              보상을 받으실 수 있습니다.
        ...
        <div className="max-w-full space-y-6 mt-10">
          <div className="space-y-6">
            ...
            {/* 추가 정보 섹션 */}
            <div className="space-y-6 pt-4 border-t border-blue-500/10">
              <h3 className="text-lg font-medium flex items-center gap-2">
              ...
              {optionalFields.map(({ key, label }) => (
                <div key={key} className="space-y-2">
                  <label className="block text-sm font-medium text-gray-300 flex items-center gap-2">
                    {label}
                    <span className="text-xs text-green-400">(선택사항)</span>
                  </label>
                  <input
                    type="text"
                    placeholder={`${label}을(를) 입력하세요`}
                    ...
                    className="w-full px-4 py-3 bg-gray-800 rounded-lg border border-white/10 
          text-white placeholder-gray-500 focus:border-blue-500 
          focus:ring-1 focus:ring-blue-500 transition-colors outline-none
